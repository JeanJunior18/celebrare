...
import { claimBulkItem } from '@/application/use-cases/claim-bulk-item.use-case';
...
export async function claimBulkItemAction(formData: FormData): Promise<GiftActionResult> {
  try {
    const repository = new PostgresGiftRepository(db);

    await claimBulkItem(repository, {
      eventId: String(formData.get('eventId') ?? ''),
      giftItemId: String(formData.get('giftItemId') ?? ''),
      guestName: String(formData.get('guestName') ?? ''),
      guestWhatsapp: formData.get('guestWhatsapp')?.toString() || undefined,
